...
**Evaluation.** The harness is measured against a labeled test set (`eval_harness.py`) — and not just for accuracy, but for *consistency*. Each case is run 3× to see whether the LLM-based verifier even agrees with itself. Cases sort into three buckets: **stable-correct** (caught reliably), **stable-wrong** (a true blind spot — missed every run), and **non-deterministic** (different verdict across identical runs). On an 11-case test set of plausible-but-wrong technical claims: **10 stable-correct, 1 stable-wrong, 0 non-deterministic.** The single blind spot is an unverifiable claim — revenue figures with no source provided — that the verifier waves through every time, because it can't flag what it has no way to check. That's the real lesson: a verifier is only as good as the grounding it's given, and the cases it gets wrong are consistent and diagnosable, not random.
